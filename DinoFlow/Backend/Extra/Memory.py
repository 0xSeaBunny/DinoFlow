...
class EpisodicMemory:
    ...
    def retrieve_relevant(self, query: str, top_k: int = 5, recency_weight: float = 0.4) -> List[Dict[str, Any]]:
        ...
        # Recency parameters
        decay_hours = 24  # Half-life in hours (memories decay to 50% after 24 hours of no access)
...
def get_episodic_memory(agent_name: str = None) -> EpisodicMemory:
    if agent_name:
        return get_agent_memory_instances(agent_name)["episodic"]
    
    global _episodic_memory
    if '_episodic_memory' not in globals():
        _episodic_memory = EpisodicMemory()
    return _episodic_memory


def get_skill_lessons(agent_name: str = None) -> SkillLessons:
    if agent_name:
        return get_agent_memory_instances(agent_name)["skills"]
    
    global _skill_lessons
    if '_skill_lessons' not in globals():
        _skill_lessons = SkillLessons()
    return _skill_lessons


def get_user_preferences(agent_name: str = None) -> UserPreferences:
    if agent_name:
        return get_agent_memory_instances(agent_name)["preferences"]
    
    global _user_preferences
    if '_user_preferences' not in globals():
        _user_preferences = UserPreferences()
    return _user_preferences


def get_pattern_detector(agent_name: str = None) -> PatternDetector:
    if agent_name:
        return get_agent_memory_instances(agent_name)["patterns"]
    
    global _pattern_detector
    if '_pattern_detector' not in globals():
        _pattern_detector = PatternDetector()
    return _pattern_detector


def get_memory_context_for_prompt(query: str, agent_name: str = None) -> str:
    parts = []
